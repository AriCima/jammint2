...
const MyJamsList = ({ flats }) => {
    const renderJams = (flats) => flats.map((f, j) => (
    ...
    const renderRooms = (jamRooms) => jamRooms.map((rooms, j) => (
        <button className="rooms-home-row" key={j} to={`/single_room_overview/${rooms.id}`}>
            ...
            <div className="rooms-home-block-name">
                {rooms.jammerName === undefined ? <p><span>Vacant</span></p> : (
                    <p>
                        {rooms.tenantName}
                        {' '}
                        {rooms.tenantSurname}
